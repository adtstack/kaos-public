...
function logSocketRejection(
	...
	console.warn(
		`[kaos-sync:worker] ws rejected pre-auth: ` +
...
export async function handleSyncSocketRoute(
	...
	if (authResult.method === "legacy-token") {
		console.warn(
			`[kaos-sync:worker] legacy ?token= WebSocket auth for vault ${vaultId.slice(0, 8)} — ` +
	...
	if (!clientSchema) {
		...
		console.warn(
			`[kaos-sync:worker] ws rejected (update_required): ` +
			JSON.stringify({
