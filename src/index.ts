...
//
// Vault resource whitelist: only known resources can proceed to auth.
...
function isKnownRecoverySnapshotRouteShape(method: string, rest: string[]): boolean {
	if (rest.length === 0) {
		return method === "GET";
	}
	if (rest.length === 1) {
...
function isKnownVaultRouteShape(method: string, resource: string, rest: string[]): boolean {
	switch (resource) {
		...
		case "recovery-snapshots":
			return isKnownRecoverySnapshotRouteShape(method, rest);

		case "recovery-content":
			return method === "GET" && rest.length === 1 && rest[0].length > 0;
...
function logWorkerRequest(args: {
	...
	// eslint-disable-next-line no-console -- structured request access log is intentionally emitted at info level.
	console.info(
		"[kaos-worker] request " + JSON.stringify({
...
const worker = {
	async fetch(req: Request, env: Env): Promise<Response> {
		...
		if (route.kind === "home") {
			...
			} else if (resource === "recovery-snapshots") {
				response = withCors(await handleRecoverySnapshotRoute(env, vaultId, req, rest, json, {
					recordVaultTrace,
				}));
			} else if (resource === "recovery-content") {
